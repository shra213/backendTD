...
exports.roomDetails = exports.getGameStatus = exports.endgame = exports.getPlayer = exports.exitGame = exports.endTurn = exports.selectTruthDare = exports.rotateBottle = exports.getRooms = exports.joinGame = exports.createGame = void 0;
...
const createGame = async (req, res) => {
    try {
        ...
        // Create room
        const roomDoc = await index_1.db.collection("rooms").add({
            roomName,
            createdBy: userid,
        ...
        await Promise.all(friendids.map(async (memId) => {
            ...
            if (index !== -1) {
                ...
                await index_1.db.collection("users").doc(otherUserId).collection("notifications").add({
            ...
            await Promise.all(friendids.map(friend => {
                const friendRef = index_1.db.collection("users").doc(friend);
        ...
        await index_1.db.collection("rooms").doc(roomDoc.id).update({
...
const joinGame = async (req, res) => {
    ...
    const userRef = index_1.db.collection("users").doc(user);
    const userDoc = await userRef.get();
    if (!userDoc.exists) {
        return res.status(404).json({ msg: "User not found" });
    }
    const roomRef = index_1.db.collection("rooms").doc(roomId);
    ...
    try {
        await roomRef.collection("players").doc(user).set({
            name: userDoc.data()?.name || "Unknown",
            ref: userRef,
            joinedAt: index_1.admin.firestore.Timestamp.now(),
        });
...
const getRooms = async (req, res) => {
    try {
        const user = req.user.uid;
        const roomsSnap = await index_1.db.collection("rooms").get();
        if (roomsSnap.empty) {
        ...
        for (const doc of roomsSnap.docs) {
            ...
            if (room.createdAt) {
                ...
                if (now - startedAtMs > twoHoursInMs) {
                    await index_1.db.collection("rooms").doc(doc.id).delete();
...
const rotateBottle = async (req, res) => {
    const roomId = req.body.id;
    if (!roomId) {
    ...
    try {
        ...
        await roomRef.update({
            currentTurn: {
                ...
                truthOrDare: null,
                task: null,
                startTime: index_1.admin.firestore.Timestamp.now(),
                endTime: null
            }
        });
...
exports.rotateBottle = rotateBottle;
const selectTruthDare = async (req, res) => {
...
exports.selectTruthDare = selectTruthDare;
const endTurn = async (req, res) => {
    ...
    try {
        ...
        const updatedTurn = {
            ...
            truthOrDare: null,
            startTime: null,
            endTime: index_1.admin.firestore.Timestamp.now()
        };
        await roomRef.update({
